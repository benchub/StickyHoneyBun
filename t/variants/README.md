...
| 039 pg_repack | ✓ | — | Docker-only: `pg_repack` heap rewrite (CREATE TABLE AS + log-table replay + catalog swap) bypasses typeoutput, so it does not fire the trap. Mechanism is variant-independent; not worth a second RDS run. Skips cleanly when the PGDG `postgresql-N-repack` package isn't installed. README's "Things to be aware of" carries a PG 19 caveat about the upcoming built-in `REPACK CONCURRENTLY`, which uses logical replication under the covers and will need re-validation |
| 040 log_level | ✓ | ✓ | Both variants: `alert_log_level` / `heartbeat_log_level` route events into PG's own log stream, and the primary sink (file on self-hosted, `lambda_arn` on RDS) can be turned off so the PG log stream is the sole sink. Self-hosted asserts both `log_path=''` (file absent) and the JSON appearing in `$node->logfile` at the configured levels; RDS asserts psql stderr carries the JSON via `RAISE` while `count_alerts` confirms zero Lambda invocations. Heartbeat routing on RDS uses the `sticky_honey_bun.heartbeat*` tag prefix |
