...
| 802 config_kill_switch | — | ✓ | RDS-only: `DELETE FROM shb_rds_internal.sticky_honey_bun_rds_config` silences the trap (parallels 007) |
...
| 806 enabled_kill_switch | — | ✓ | RDS analog of self-hosted 007: the locked-down `shb_rds_internal.sticky_honey_bun_rds_config` row `enabled='off'` silences the trap; only the extension owner can flip it (an app role's UPDATE attempt is permission-denied) |
