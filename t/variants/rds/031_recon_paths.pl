...
# join pg_attribute against honey_bun_registry (which we DID lock down)
# to reconstruct the column list — BUT only if they can read the
# registry. Verify both halves.
{
    # registry is also REVOKEd from PUBLIC
    my ($rc, undef, $stderr) = $as_app->(
        'SELECT count(*) FROM honey_bun_registry');
    isnt($rc, 0,
        'recon: honey_bun_registry also locked down from PUBLIC');
    like($stderr, qr/permission denied/i,
