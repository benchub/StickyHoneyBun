...
#          sticky_honey_bun_rds_config table is a working kill switch.
...
my (undef, $saved_enabled) = SHB_RDS::psql_run($cs_config,
    "SELECT coalesce(value, '__missing__') FROM sticky_honey_bun_rds_config WHERE key = 'enabled' "
...
END {
    if ($saved_enabled eq '__missing__') {
        SHB_RDS::psql_run($cs_config,
            "DELETE FROM sticky_honey_bun_rds_config WHERE key = 'enabled'");
    } else {
        SHB_RDS::psql_run($cs_config,
...
# Flip the kill switch: set `enabled` to 'off'.
{
    my ($rc) = SHB_RDS::psql_run($cs_config,
        "INSERT INTO sticky_honey_bun_rds_config(key, value) VALUES ('enabled', 'off') "
      . "ON CONFLICT (key) DO UPDATE SET value = EXCLUDED.value");
...
{
    my ($rc) = SHB_RDS::psql_run($cs_config,
        "UPDATE sticky_honey_bun_rds_config SET value = 'on' WHERE key = 'enabled'");
...
# Verify the lockdown: an app role cannot flip the switch.
{
    ...
    my ($rc, undef, $stderr) = SHB_RDS::psql_run($cs_app,
        "UPDATE sticky_honey_bun_rds_config SET value = 'off' WHERE key = 'enabled'");
    isnt($rc, 0,
